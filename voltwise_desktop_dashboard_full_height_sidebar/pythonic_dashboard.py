...
# -*- coding: utf-8 -*-
"""
VoltWise Action Dashboard
En renere ("Pythonic") version af main.py, som opretholder det originale Tailwind-design.
"""
...
def setup_head():
    """Opsætter Tailwind, Google Fonts og evt. custom CSS"""
    ui.add_head_html('''
    ...
    with ui.element('aside').classes('w-72 bg-[#f8faf9] border-r border-nordic-gray h-screen sticky top-0 flex flex-col p-8 flex-shrink-0'):
        ...
        with ui.element('nav').classes('flex-1 space-y-2'):
            ...
            for item in nav_items:
                if item['active']:
                    link_class = 'flex items-center gap-3 px-4 py-3 bg-primary/10 text-primary-dark rounded-xl font-bold transition-colors'
                else:
                    link_class = 'flex items-center gap-3 px-4 py-3 text-gray-500 hover:bg-background-light hover:text-[#0d1c12] rounded-xl font-semibold transition-colors'
                    
                with ui.link(target='#').classes(link_class):
                    ui.label(item['icon']).classes('material-symbols-outlined')
                    ui.label(item['label'])
...
def create_top_header():
    ...
    with ui.element('header').classes('flex items-center justify-between mb-12 w-full'):
        ui.element('div').classes('flex items-center gap-6') # Tom space til evt. breadcrumbs
        
        with ui.element('div').classes('flex items-center gap-4'):
            with ui.button(color=None).props('unelevated no-caps').classes('!p-2 hover:bg-background-light rounded-full transition-colors'):
            ...
            ui.element('div').classes('w-10 h-10 rounded-full bg-nordic-gray overflow-hidden border border-nordic-gray').style(f"background-image: url('{avatar_url}'); background-size: cover;")
...
def create_hero_section():
    ...
    with ui.element('section').classes('relative py-20 rounded-xl flex flex-col items-center justify-center text-center status-glow'):
        ...
        # Opdater knap
        with ui.button(color=None, on_click=lambda: ui.notify('Opdaterer...', type='info')).props('unelevated no-caps').classes('mt-10 flex items-center gap-2 bg-[#0d1c12] text-white !px-8 !py-4 !min-h-0 rounded-xl font-bold hover:scale-105 transition-transform'):
...
def create_smart_delay_section():
    ...
    with ui.element('section'):
        ...
        with ui.element('div').classes('flex items-center justify-between mb-8'):
            ...
            with ui.button(color=None).props('unelevated no-caps ripple="false"').classes('text-primary-dark font-bold text-sm flex items-center gap-1 hover:underline'):
                ui.label('Se alle enheder')
                ui.label('arrow_forward').classes('material-symbols-outlined text-sm')
                
        # Data for enheder
        devices = [
            {'icon': 'dishwasher', 'name': 'Opvaskemaskine', 'price_now': '4,50 kr.', 'price_later': '1,20 kr.', 'saving': '3,30 kr.'},
            {'icon': 'dry_cleaning', 'name': 'Tørretumbler', 'price_now': '6,20 kr.', 'price_later': '2,10 kr.', 'saving': '4,10 kr.'},
            {'icon': 'ev_station', 'name': 'Elbil', 'price_now': '45,00 kr.', 'price_later': '15,50 kr.', 'saving': '29,50 kr.'}
        ]
        
        # Enheds-kort i et grid
            ...
            for device in devices:
                with ui.element('div').classes('bg-white border border-nordic-gray rounded-xl p-6 flex flex-col hover:shadow-xl hover:shadow-primary/5 transition-all'):
                    with ui.element('div').classes('w-12 h-12 bg-background-light rounded-lg flex items-center justify-center mb-6'):
                        ui.label(device['icon']).classes('material-symbols-outlined text-primary-dark')
                    
                    ui.label(device['name']).classes('text-lg font-bold mb-4')
                    ...
                    with ui.element('div').classes('space-y-3 mb-8'):
                        with ui.element('div').classes('flex justify-between text-sm'):
                            ...
                            ui.label(device['price_now']).classes('font-semibold')
                        with ui.element('div').classes('flex justify-between text-sm'):
                            ui.label('Vent og spar').classes('text-gray-500')
                            ui.label(device['price_later']).classes('font-semibold text-primary-dark')
                            
                    # Knap og besparelse (bund)
                    with ui.element('div').classes('mt-auto space-y-3'):
                        ui.label(f"Spar {device['saving']}").classes('bg-primary/10 border border-primary/30 text-primary-dark text-xs font-bold py-1 px-3 rounded-full w-fit')
                        # Falsk knap via styling for at undgå uønsket NiceGUI-knap styling (hvis du foretrækker rigtig knap, brug ui.button som på opdater-knappen)
                        with ui.element('button').classes('w-full bg-[#0d1c12] text-white py-3 rounded-lg font-bold hover:bg-opacity-90 transition-colors text-center'):
                            ui.label('Vent til kl. 01:00')

def create_social_section():
    """Dele-funktion / Social Gamification"""
    with ui.element('section').classes('bg-background-light p-8 rounded-xl flex items-center justify-between'):
        ...
        with ui.button(color=None).props('unelevated no-caps').classes('bg-primary text-black !px-8 !py-3 !min-h-0 rounded-xl font-bold flex items-center gap-2 hover:scale-105 transition-transform'):
        ...
        with ui.element('div').classes('bg-background-light rounded-xl p-8 border border-nordic-gray sticky top-8'):
            ...
            with ui.element('div').classes('mb-10'):
                ...
                with ui.element('div').classes('mt-4 h-2 bg-nordic-gray rounded-full overflow-hidden'):
                    ui.element('div').classes('bg-primary h-full w-[65%]')
